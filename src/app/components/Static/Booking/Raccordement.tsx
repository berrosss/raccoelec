...
const Raccordement = () => {
  ...
  const validationSchemaForm3 = yup.object().shape({
    ...
    viabilise: yup.string().required("Veuillez remplire ce champ."),
    puissance: yup.string().when([], {
      is: !isCheckboxChecked,
        then: (schema) => schema.required("Veuillez choisir une puissance."),
        otherwise: (schema) => schema.notRequired(),
    })
  });
  
  const validationSchemaForm4 = yup.object().shape({
    portes: yup.string().required("Veuillez remplire ce champ."),
    echeance: yup.string().required("Veuillez remplire ce champ."),
    autorisation: yup.string().required("Veuillez remplire ce champ."),
  });
  ...
  const methods = useForm<FormData>({
    resolver: yupResolver(
      ...
      currentStep === 4 ? validationSchemaForm4 : 
      yup.object().shape({})
    ),
  });
  ...
  const handleRadioChange = (value: string) => {
    setSelectedPuissance(value);
    setIsCheckboxChecked(false);
    methods.setValue("puissance", value, { shouldValidate: true });
  };

  const handleCheckboxChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setIsCheckboxChecked(event.target.checked);
    if (event.target.checked) {
      setSelectedPuissance("Je ne connais pas mon besoin");
      methods.setValue("puissance", "Je ne connais pas mon besoin", { shouldValidate: true });
    } else {
      setSelectedPuissance(null);
      methods.setValue("puissance", "", { shouldValidate: true });
    }
  };
              ...
              <div className="mb-8 mt-10">
                <div className="flex items-center justify-center mt-10">
                  <div className="flex items-center text-blue-700 space-x-7">
                    ...
                    <span>&gt;</span>
                    <div className={`step ${ (currentStep === 3 || currentStep === 4) ? "active" : "" }`} id="step-3">
                    ...
                    <span>&gt;</span>
                    <div className={`step ${ currentStep == 5 ? "active" : "" }`} id="step-4">
              ...
              <form onSubmit={methods.handleSubmit(onSubmit)} className="w-full p-[40px] rounded" style={{ boxShadow: "0 10px 30px 0 rgba(62, 87, 111, 0.2)" }}>
                          ...
                          <div className="flex justify-start items-start flex-col md:flex-row w-full gap-2 text-center">
                            {['Viabilisation de terrain', 'Raccordement provisoire', 'Raccordement définitif'].map((option, index) => (
                              <div key={index} className="w-full lg:w-auto">
                                ...
                                <label
                                  htmlFor={`need${index}`}
                                  className="viab transition-all duration-300 hover:border-[#1623dc] hover:text-[#1623dc] inline-flex items-center justify-between w-full p-3 text-gray-500 rounded bg-white border-[1px] border-gray-400 cursor-pointer peer-checked:text-black peer-checked:border-[#005EB8] peer-checked:bg-[#bfd7ed]">
                  ...
                  {currentStep === 3 && (
                    <div>
                        ...
                        <h2 className="stepper-title mt-14 text-[16px] leading-[24px] font-light text-left text-[#212529] mb-10">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 space-y-4 lg:space-y-0 lg:space-x-4 mb-4">
                          {[
                            { 'label': 'Oui, mon terrain est viabilisé', 'img': '/assets/formulaire/terrain_viabilise.jpg', "description": "Exemple de terrain viabilisé" },
                            { 'label': 'Non, mon terrain n‘est pas', 'img': '/assets/formulaire/terrain_non_viabilise.webp', "description": "Exemple de terrain non viabilisé" }
                          ].map((option, index) => (
                            <label htmlFor={`viabilise${index}`} className="flex flex-col w-full items-center cursor-pointer" key={index}>
                              <div className="relative w-full">
                                <input
                                  type="radio"
                                  {...methods.register("viabilise")}
                                  id={`viabilise${index}`}
                                  name="viabilise" // Ensure all radio buttons have the same name
                                  value={option.label}
                        ...
                        <div className="mb-14">
                            {methods.formState.errors.viabilise && <p className="text-red-500 text-sm">{methods.formState.errors.viabilise.message}</p>}
                        </div>
                        ...
                        <div className="flex items-center justify-start space-x-5 border-2 border-slate-200 rounded py-3 px-2 overflow-scroll lg:overflow-none">
                          {[3, 6, 9, 12, 15, 18, 21, 24, 27, 30, 36, "Plus de 36"].map((number, index) => (
                            <label key={index} className={`relative cursor-pointer ${number === "Plus de 36" ? "ml-5" : ""}`}>
                              <input
                                type="radio"
                                {...methods.register("puissance")}
                                name="puissance"
                                value={String(number)}
                                className="hidden"
                                onChange={() => handleRadioChange(String(number))}
                        ...
                        <div>
                          {methods.formState.errors.puissance && <p className="text-red-500 text-sm">{methods.formState.errors.puissance.message}</p>}
                        </div>
                        <fieldset>
                  ...
                  {currentStep === 4 && (
                      <div>
                          ...
                          <div className="grid grid-cols-1 lg:grid-cols-2 space-y-4 lg:space-y-0 lg:space-x-4 ">
                              {[
                                { 'label': 'Oui', 'img': '/assets/formulaire/fenetre_installe.webp', "description" : "les portes extérieures et fenêtres sont installées" },
                                { 'label': 'Non', 'img': '/assets/formulaire/fenetre_no_installe.jpeg', "description" : "les portes et fenêtres ne sont pas installées" }
                              ].map((option, index) => (
                                <label htmlFor={`portes${index}`} className="flex flex-col w-full items-center cursor-pointer" key={index}>
                                  <div className="relative w-full">
                                    <input
                                      type="radio"
                                      {...methods.register("portes")}
                                      id={`portes${index}`}
                                      name="portes"
                                      value={option.label}
                                      className="sr-only peer"
                                    />
                                    <div className="viab2 transition-all duration-300 w-full h-[320px] rounded-lg p-3 border-2 border-gray-300 peer-checked:border-[#16a974] overflow-hidden">
                                      <div className="text-black text-sm text-center mb-0.5">
                                        {option.label}
                                      </div>
                          ...
                          <div>
                            {methods.formState.errors.portes && <p className="text-red-500 text-sm">{methods.formState.errors.portes.message}</p>}
                          </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                {["Oui, je l'ai déjà", "Non, je ne l'ai pas encore", "Je n'en ai pas besoin"].map((option, index) => (
                                  <div key={index}>
                                    <label htmlFor={`autorisation${index}`} className="viab transition-all duration-300 flex cursor-pointer items-center justify-center gap-4 rounded border border-slate-400 bg-white p-4 text-md font-medium shadow-sm hover:border-slate-500 has-[:checked]:border-blue-500 has-[:checked]:ring-1 has-[:checked]:ring-blue-500 has-[:checked]:bg-blue-100">
                                      <p className="text-gray-700">{option}</p>
                                      <input
                                        type="radio"
                                        {...methods.register("autorisation")}
                                        name="autorisation"
                                        value={option}
                          ...
                          <div>
                              {methods.formState.errors.autorisation && <p className="text-red-500 text-sm">{methods.formState.errors.autorisation.message}</p>}
                          </div>
                          ...
                          <div className="flex justify-end items-center gap-3 mt-10">
                            <button onClick={() => {
                             window.scrollTo({top: 0,behavior: "smooth"});
                             setCurrentStep(currentStep - 1);
                          }}  id="prev3" type="button" className="bg-white border-[1px] border-[#16a974] rounded text-[#16a974] py-2.5 px-10 text-md font-semibold">
                              Précédent
                            </button>
                            <button id="next3" type="submit" className="bg-blue-600 border-[1px] border-blue-600 rounded text-white py-2.5 px-10 text-md font-semibold">
                              Suivant
                            </button>
                          </div>
                      ...
                      <div className="mb-10">
                        ...
                        <ul className="mb-8">
                          ...
                          <li className="gap-3 mb-3gap-3">
                            ...
                            Autorisation d&#39;urbanisme: <strong>{formData.autorisation}</strong>
                          </li>
                        </ul>
                      </div>
